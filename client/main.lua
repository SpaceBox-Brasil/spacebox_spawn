...
local lastSentIconSignature = nil

local function roundMapIconPosition(value)
    return math.floor((value * 1000) + 0.5) / 1000
end

local function getMapIconSignature(allIcons)
    if not allIcons or #allIcons == 0 then
        return 'empty'
    end
...
local function sendMapIcons(allIcons, force)
    local signature = getMapIconSignature(allIcons)

    if not force and signature == lastSentIconSignature then
        return
    end

    lastSentIconSignature = signature
    SendNUIMessage({
        action = 'updateMapIcon',
        allIcons = allIcons or {}
    })
end
...
local function startMarkerThread()
    ...
    CreateThread(function()
        while markerThread and isNuiOpen do
                    ...
                    if spawn and spawn.coords then
                        ...
                        if x and y and z then
                            ...
                            if iconOnScreen and icon_x and icon_y and icon_x >= 0.0 and icon_x <= 1.0 and icon_y >= 0.0 and icon_y <= 1.0 then
                                local xPos = roundMapIconPosition(math.max(0.0, math.min(1.0, icon_x)))
                                local yPos = roundMapIconPosition(math.max(0.0, math.min(1.0, icon_y)))
            ...
            if not isCameraAnimating then
                local maxVisibleMapIcons = config.maxVisibleMapIcons or 12
                if #allIcons > maxVisibleMapIcons then
                    local limitedIcons = {}

                    for i = 1, #allIcons do
                        if allIcons[i].selected then
                            limitedIcons[#limitedIcons + 1] = allIcons[i]
                            break
                        end
                    end

                    for i = 1, #allIcons do
                        if #limitedIcons >= maxVisibleMapIcons then break end
                        if not allIcons[i].selected then
                            limitedIcons[#limitedIcons + 1] = allIcons[i]
                        end
                    end

                    allIcons = limitedIcons
                end
            end

            -- Enviar todos os ícones de uma vez para React
            sendMapIcons(allIcons, false)
            
            if isCameraAnimating then
                Wait(120)
            else
                Wait(config.mapMarkerUpdateInterval or 120)
            end
        end
        markerThread = false
    end)
...
local function stopMarkerThread()
    ...
    lastSentIconSignature = nil
    -- Ocultar todos os icones
    sendMapIcons({}, true)
end
...
RegisterNUICallback('selectSpawn', function(data, cb)
    ...
    -- Ocultar icone durante a animacao sem gerar payload legado repetido.
    sendMapIcons({}, true)
...
RegisterNUICallback('confirmSpawn', function(_, cb)
    ...
    zoomCameraToPlayer(spawnData.coords, spawnData, config.zoomDuration or 4000, function(spawnInfo)
        ...
        TriggerServerEvent('spacebox_multichar:server:setBucket', 0)
...
RegisterNUICallback('close', function(data, cb)
    ...
    if returnToMultichar then
        ...
        if GetResourceState('spacebox_multichar'):find('start') then
            exports['spacebox_multichar']:openMultichar()
        else
